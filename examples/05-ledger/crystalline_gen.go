...
import (
	...
	ledger "github.com/Vilsol/crystalline/examples/05-ledger/ledger"
)

func init() {
	if crystallineImported, err := crystallineResolvePath("localStorage"); err != nil {
		crystallineImportFailed("ledger.Saved", err)
	} else if crystallineFilled, err := crystallineImportLedgerStore(crystallineImported); err != nil {
		crystallineImportFailed("ledger.Saved", err)
	} else {
		ledger.Saved = crystallineFilled
	}

	crystallinePublishImportFailures("ledger")

	crystallineNamespace("ledger", "ledger").Set("add", crystallineWrap(js.FuncOf(crystallineFnLedgerAdd)))
	crystallineNamespace("ledger", "ledger").Set("balance", crystallineWrap(js.FuncOf(crystallineFnLedgerBalance)))
	crystallineNamespace("ledger", "ledger").Set("forget", crystallineWrap(js.FuncOf(crystallineFnLedgerForget)))
...
func crystallineFnLedgerAdd(this js.Value, args []js.Value) (result any) {
	defer crystallineRecover(&result)
	...
	return crystallineMarshalLedgerEntry(&r0)
}

func crystallineFnLedgerBalance(this js.Value, args []js.Value) (result any) {
	defer crystallineRecover(&result)

	if len(args) != 0 {
		return crystallineFail("Balance: expected 0 arguments, got " + strconv.Itoa(len(args)))
	}
...
func crystallineFnLedgerForget(this js.Value, args []js.Value) (result any) {
	defer crystallineRecover(&result)

	if len(args) != 0 {
		return crystallineFail("Forget: expected 0 arguments, got " + strconv.Itoa(len(args)))
	}
...
func crystallineMarshalLedgerEntry(v *ledger.Entry) any {
	if v == nil {
		return nil
	}
	...
	crystallineDefine(scope, out, "id", func() any {
		return crystallineBigInt(int64(v.ID))
	}, func(value js.Value) {
		converted, err := crystallineToBigInt(value)
		if err != nil {
		...
		v.ID = int64(converted)
	})
	crystallineDefine(scope, out, "description", func() any {
	...
	crystallineDefine(scope, out, "amount", func() any {
...
type crystallineImportedStore struct{ value js.Value }
...
func (c crystallineImportedStore) SetItem(p0 string, p1 string) {
	crystallineDirect(c.value.Call("setItem", string(p0), string(p1)), "ledger.Store.SetItem")
}

func crystallineImportLedgerStore(value js.Value) (ledger.Store, error) {
	if value.Type() != js.TypeObject {
		return nil, errors.New("ledger.Store: expected an object")
	}

	if value.Get("getItem").Type() != js.TypeFunction {
		return nil, errors.New("ledger.Store: the object has no getItem method")
	}

	if value.Get("setItem").Type() != js.TypeFunction {
		return nil, errors.New("ledger.Store: the object has no setItem method")
	}

	return crystallineImportedStore{value: value}, nil
}
